...
public class FingerTip2Script : MonoBehaviour {
    ...
    void Update ()
    {
		...
		if (!frame.Hands.Empty)
		{
			...
			if (!fingers.Empty)
			{
				...
				Finger minFig = fingers[0];
				Finger maxFig = fingers[0];
    			...
    			foreach (Finger finger in fingers)
    			{
					...
					else if(finger.TipPosition.x > tarFig.TipPosition.x)
					{
						maxFig = finger;
					}
				}
				...
				foreach (Finger finger in fingers)
    			{
					if(Mathf.Abs(finger.TipPosition.x-((maxFig.TipPosition.x+minFig.TipPosition.x)/2))<Mathf.Abs(tarFig.TipPosition.x-((maxFig.TipPosition.x+minFig.TipPosition.x)/2)))
					{
						tarFig = finger;
					}
				}
